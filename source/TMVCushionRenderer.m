...
#define MAX_RGB_VALUE 1

NSColor *defaultCushionColor = nil;
...
- (id) init
{
    ...
    if ( defaultCushionColor != nil )
    {
        defaultCushionColor = [TMVCushionRenderer normalizeColor: [NSColor colorWithCalibratedRed: 0 green: 0 blue: 0.9 alpha: 1]];
        [defaultCushionColor retain];
    }

    _color = defaultCushionColor;
    [_color retain];
...
Unoptimized:
    ...
    double h4= 4 * heightFactor;

    double wf= h4 / NSWidth(_rect);
    _surface[2]+= wf * ( NSMaxX(_rect) + NSMinX(_rect) );
    _surface[0]-= wf;

    double hf= h4 / NSHeight(_rect);
...
- (void) renderCushionInBitmap: (NSBitmapImageRep*) bitmap
{
    ...
    const double *surface = [self surface];
    ...
    for ( iy = (int) NSMinY(rect); iy < (int) NSMaxY(rect); iy++)
    {
        unsigned char *rowStart = pixels + iy * bytesPerRow;

        for ( ix = (int) NSMinX(rect); ix < (int) NSMaxX(rect); ix++)
        {
            ...
            /*
             //with QuickDraw, this would be nicer:
...
{
    NSAssert(*red + *green + *blue <= 3 * MAX_RGB_VALUE, @"color error");
...
+ (NSColor*) normalizeColor: (NSColor*) color
{
    ...
    double f= BASE_BRIGHTNESS / (red + green + blue);
    red *= f;
    green *= f;
    blue *= f;
