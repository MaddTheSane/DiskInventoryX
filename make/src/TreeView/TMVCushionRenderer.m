...
- (void) renderCushionInBitmapPPC603Single: (NSBitmapImageRep*) bitmap; //PowerPC optimzed version (603+)
{
    ...
    for ( iy = fiy = NSMinY(rect); iy < yEnd ; iy++, fiy++)
    {
        ...
        for ( ix = xStart, fix = NSMinX(rect); ix < xEnd ; ix++, fix++)
        {
			...
			cosa *= nx*Lx + ny*Ly + Lz;
#else
			register float cosa = 1 / sqrt(sqrtParam);
#endif
...
+ (NSColor*) normalizeColor: (NSColor*) color
{
    ...
    CGFloat f= componentSum != 0 ? (BASE_BRIGHTNESS / componentSum) : 1;
    red *= f;
    green *= f;
    blue *= f;
...
+ (void) distributeRGB1: (CGFloat*) first toRGB2: (CGFloat*) second toRGB3: (CGFloat*) third
{
    ...
    *third += h;
    ...
    {
        CGFloat h = *second - MAX_RGB_VALUE;
