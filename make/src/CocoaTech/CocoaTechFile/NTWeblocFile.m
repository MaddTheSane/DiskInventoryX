...
{
    int mapVersion;  // always 1
    int unused1;     // always 0
    int unused2;     // always 0
    short unused;
...
+ (id)entryWithType:(OSType)type resID:(ResID)resID;

@property (readonly) OSType type;
@property (readonly) ResID resID;
- (NSData*)entryData;
...
@property (readwrite, retain, setter=setURL:) NSURL *url;
    ...
    if (![NTPathUtilities pathOK:path])
    {
        ...
        if ([desc isValid])
        {
            ...
            else if (_attributedString)
            {
				...
				data = [_attributedString RTFFromRange:NSMakeRange(0,[_attributedString length]) documentAttributes:@{}];
...
+ (NSArray*)readEntriesFromWeblocFile:(NTFileDesc*)desc;
{
    ...
    {
        NSInteger totalLength=[dragData length];
        NSInteger offset=0;
        NSInteger dataLen;
...
- (id)initWithType:(OSType)type resID:(ResID)resID;
{
    self = [super init];
