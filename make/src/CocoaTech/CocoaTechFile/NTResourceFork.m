    ...
    if( self = [self init] )
    {
        ...
        if ((aPermission & 0x06) != 0)		// if write permission
        {
            if ( [[aPath stringByDeletingLastPathComponent] getFSRef:&theParentFsRef] )
            {
				...
				// FSCreateResFile is gay, can't handle colons in the name 
				theName = [theName stringByReplacing:@":" with:@"/"];
...
- (NSString *)stringForType:(ResType)aType Id:(short)anID
{
    ...
    return theData ? [NSString stringWithUTF8String:([theData bytes] + 1) length:[theData length] - 1] : nil;
}
