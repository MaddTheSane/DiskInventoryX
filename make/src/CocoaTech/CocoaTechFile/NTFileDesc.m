		...
		@synchronized(self) {			
			...
			if ([self isPackage])
			{
				...
				if (bundle)
				{
					...
					if (!result)
					{
						NSDictionary *dict2 = [bundle infoDictionary];
						if (dict2)
							result = [dict2 objectForKey:@"CFBundleShortVersionString"];
					}
				}
			}
			...
			if ([self isPackage])
			{
				...
				if (bundle)
				{
					...
					if (!result)
					{
						NSDictionary *dict2 = [bundle infoDictionary];
						if (dict2)
							result = [dict2 objectForKey:@"CFBundleVersion"];
					}
				}
			}
			...
			if ([self isPackage])
			{
				...
				if (bundle)
				{
					...
					if (!result)
					{
						NSDictionary *dict2 = [bundle infoDictionary];
						if (dict2)
							result = [dict2 objectForKey:@"CFBundleGetInfoString"];
					}
				}
			}
...
- (NSArray*)pathComponents:(BOOL)resolveAliases;
{
    ...
    NSInteger i, cnt = [pathComponents count];
...
- (NSString*)itemInfo;
{
	...
	if (![[self cache] itemInfo_initialized:&result])
	{
		@synchronized(self) {
			...
			{
				result = [[self metadata] displayValueForAttribute:(NSString*)kMDItemDurationSeconds];//[self size]
				
				if ([result length] && [self isFile])  // return the size for normal files
					result = [NSString stringWithFormat:@"%@ / %@", result, [NSByteCountFormatter stringFromByteCount:[self size] countStyle:NSByteCountFormatterCountStyleFile]];
			}
			...
			{
				if ([self isFile])  // return the size for normal files
					result = [NSByteCountFormatter stringFromByteCount:[self size] countStyle:NSByteCountFormatterCountStyleFile];
			}
