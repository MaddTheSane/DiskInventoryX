...
+ (NSImage*)iconRef:(IconRef)iconRef toImage:(int)size label:(int)label select:(BOOL)select alpha:(CGFloat)alpha;
{
	return [NSImage iconRef:iconRef toImage:size label:label select:select alpha:alpha alignment:kAlignAbsoluteCenter];
}
		  ...
		  alignment:(int)alignment;
{
	NTImageMaker* maker;
	@autoreleasepool {
		maker = [[NTImageMaker maker:NSMakeSize(size,size)] retain];
	}
	...
	NSImage *maker1 = [[maker unlockFocus] retain];
	[maker release];
	
	@autoreleasepool {
		maker = [NTImageMaker maker:NSMakeSize(size * 2,size * 2)];
		[maker lockFocus];
		[NTIconCompositor drawIconForRef:iconRef inRect:NSMakeRect(0,0,size*2,size*2) label:label selected:select opened:NO flip:NO alpha:alpha alignment:alignment];
		NSImage *maker2 = [maker unlockFocus];
		NSBitmapImageRep *imgRep = (NSBitmapImageRep *)[maker2 representations].firstObject;
		if (imgRep && [imgRep isKindOfClass:[NSBitmapImageRep class]]) {
			imgRep.size = NSMakeSize(size, size);
			[maker1 addRepresentation:imgRep];
		}
	}

	
	return [maker1 autorelease];
}
