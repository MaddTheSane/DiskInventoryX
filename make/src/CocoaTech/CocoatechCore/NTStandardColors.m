...
+ (NSColor*)sourceListBackgroundColor:(BOOL)dimmed;
{
	if (dimmed)
	{
		...
		if (!shared)
			shared = [NSColor sourceListBackgroundColor]; // [[NSColor colorWithCalibratedRed:.91 green:.91 blue:.91 alpha:1] retain];
    ...
    if (!shared)
        shared = [NSColor sourceListBackgroundColor]; // [[NSColor colorWithCalibratedRed:.820 green:.843 blue:.886 alpha:1] retain];
