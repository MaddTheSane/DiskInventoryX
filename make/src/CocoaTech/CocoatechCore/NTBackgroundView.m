			...
			imageDrawingMode:(NTImageDrawingMode)imageDrawingMode;
{
	...
	else if (imagePath)
	{
		...
		NS_DURING
			image2 = [[NSImage alloc] initWithContentsOfFile:imagePath];
		NS_HANDLER
			image2 = nil;
		NS_ENDHANDLER
		...
		if (image2)
		{
			[image2 normalizeSize];  // Dan Woods suggestion?
			
			[image2 setCacheMode:NSImageCacheNever];
			//[image2 setScalesWhenResized:YES];
			[self drawBackgroundImageInView:view
								   clipRect:clipRect
									  image:image2 
								   fraction:fraction
