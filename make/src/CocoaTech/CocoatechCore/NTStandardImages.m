...
+ (NSImage*)chevron;
{
    ...
    if (!shared)
        shared = [[NSBundle bundleForClass:self] imageWithName:@"NTChevron.tiff" inDirectory:@"images"];
...
+ (NSImage*)favoritesTemplate;
{
    ...
    if (!shared)
	{
        shared = [[NSBundle bundleForClass:self] imageWithName:@"favorites.png" inDirectory:@"images"];
...
+ (NSImage*)actionGear;
{
    ...
    if (!shared)
	{
		shared = [NSImage imageNamed:NSImageNameActionTemplate];
