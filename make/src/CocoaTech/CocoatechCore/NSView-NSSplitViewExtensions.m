...
- (NSMenuItem*)splitViewMenuItem;
{
	...
	if (splitView)
	{
		...
		for (NSView *subview in [splitView subviews])
		{
			if ([subview isCollapsed])
			{
				NSMenuItem* menuItem = [[NSMenuItem alloc] initWithTitle:[NTLocalizedString localize:@"Reveal Collapsed View" table:@"menuBar"] action:@selector(uncollapseSplitViewAction:) keyEquivalent:@""];
