...
{
	dataBuffer->data = CFDataCreateMutable(NULL, 0);
...
NTDataBufferSetCapacity(NTDataBuffer *dataBuffer, size_t capacity)
{
    ...
    dataBuffer->buffer = (unsigned char *)CFDataGetMutableBytePtr(dataBuffer->data);
...
NTDataBufferAppendString(NTDataBuffer *dataBuffer, CFStringRef string, CFStringEncoding encoding)
{
    ...
    if (charactersWritten != characterCount) {
        [NSException raise: NSInternalInconsistencyException
                    format: @"NTDataBufferAppendString was supposed to write %ld characters but only wrote %ld", characterCount, charactersWritten];
    }
...
NTDataBufferAppendBytecountedUTF8String(NTDataBuffer *dataBuffer, NTDataBuffer *scratchBuffer, CFStringRef string)
{
    ...
    if (charactersWritten != stringLength)
        [NSException raise: NSInternalInconsistencyException
                    format: @"NTDataBufferAppendBytecountedUTF8String was supposed to write %ld characters but only wrote %ld", stringLength, charactersWritten];
...
NTDataBufferAppendUnicodeString(NTDataBuffer *dataBuffer, CFStringRef string)
{
    ...
    if (charactersWritten != characterCount) {
        [NSException raise: NSInternalInconsistencyException
                    format: @"NTDataBufferAppendUnicodeString was supposed to write %ld characters but only wrote %ld", characterCount, charactersWritten];
    }
