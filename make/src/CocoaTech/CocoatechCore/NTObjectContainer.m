...
- (id)initWithCap:(UInt8)capacity mapCap:(UInt8)mapCapacity;
{
	...
	self->objects = (__strong id*)calloc(capacity, sizeof(id));
...
{		
	...
	if (mappedIndex < self->dataLength)
	{
		...
		self->objects[mappedIndex] = theObject;
...
- (void)resizeMemory:(UInt8)length;
{
	...
	{		
		...
		self->objects = (__strong id*)realloc(self->objects, newMemoryLength * sizeof(id));
