...
- (NSImage*)imageWithShadow:(NSShadow*)shadow;
{
	...
	[self drawAtPoint:NSMakePoint(0,0) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0];
...
- (NSImage*)coloredImage:(NSColor*)color;
{
	...
	[self drawAtPoint:NSZeroPoint fromRect:NSZeroRect operation:NSCompositeDestinationIn fraction:1.0];
...
- (NSImage*)imageWithAlpha:(CGFloat)theAlpha;
{
	...
	[self drawAtPoint:NSZeroPoint fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:theAlpha];
...
- (NSImage*)imageWithTopMargin:(NSUInteger)topMargin bottomMargin:(NSUInteger)bottomMargin;
{
	...
	[self drawAtPoint:destRect.origin fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1];
...
- (NSImage*)sizeIcon:(int)size;
{
    ...
    for (__strong NSImageRep *rep in reps)
    {
        if (([rep size].width == size) && ([rep size].height == size))
        {
			sizedImage = [[NSImage alloc] initWithSize:NSMakeSize(size, size)];
...
{	
	...
	[controlImage drawAtPoint:arrowRect.origin fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0];
...
+ (NSImage*)image:(NSRect)bounds interior:(NSImage*)interior backColor:(NSColor*)backColor;
{
	...
	[interior drawAtPoint:interiorBounds.origin fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0];
