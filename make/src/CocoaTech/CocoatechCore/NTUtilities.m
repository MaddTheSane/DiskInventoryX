...
+ (NSString *)computerName
{
	...
	if (name)
	{
		computerName=(NSString*)CFBridgingRelease(name);
	}
...
+ (NSString*)usersEmailAddress;
{
	...
	if (me)
	{
		...
		NSUInteger valueIndex = [emailAddresses indexForIdentifier:[emailAddresses primaryIdentifier]];
...
{
	NSString* result = (NSString*) CFBridgingRelease(UTCreateStringForOSType((OSType) intValue));
...
+ (NSString*)ipAddress:(NSString**)outInterface;
{
	...
	if (store)
	{
		...
		if (interfacesKey)
		{
			dictRef = (NSDictionary*)CFBridgingRelease(SCDynamicStoreCopyValue(store, interfacesKey));
			
			if (dictRef)
			{
				interfaces = [dictRef objectForKey:(NSString *)kSCDynamicStorePropNetInterfaces];
			}
		...
		{			
			...
			if (stringRef)
			{
				linkKey = (NSString*)CFBridgingRelease(stringRef);
			}
				
			NSNumber *activeValue = nil;
			dictRef = CFBridgingRelease(SCDynamicStoreCopyValue(store, (CFStringRef)linkKey));
			if (dictRef)
			{
				activeValue = [dictRef objectForKey:(NSString *)kSCPropNetLinkActive];
			}
			...
			{			
				...
				if (stringRef)
				{
					dictRef = CFBridgingRelease(SCDynamicStoreCopyValue(store, stringRef));
					
					if (dictRef)
					{
						ipAddresses = [dictRef objectForKey:(NSString *)kSCPropNetIPv4Addresses];
					}
