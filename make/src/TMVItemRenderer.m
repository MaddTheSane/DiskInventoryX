...
#define  CUSHION_SCALE_FACTOR	 0.9
...
//============================ interface TMVItemRenderer( Private ) =============================================

@interface TMVItemRenderer( Private )

- (void) drawCushionInBitmap: (NSBitmapImageRep*) bitmap parentCushion: (TMVCushionRenderer*) parentCushion cushionHeightFactor: (double) heightFactor;
...
//============================ implementation TMVItemRenderer =============================================
...
{
    [self drawCushionInBitmap: bitmap parentCushion: nil cushionHeightFactor: 0.5];
}
...
- (TMVItemRenderer *) hitTest: (NSPoint) aPoint
{
    ...
    else
    {
        ...
        while ( ( childRenderer = [childEnum nextObject] ) != nil )
        {
            TMVItemRenderer *hittedChildRenderer = [childRenderer hitTest: aPoint];            
...
//============================ implementation TMVItemRenderer( Private ) =============================================
...
- (void) layoutChilds
{
    ...
    for ( row = 0; row < [rows count]; row++ )
    {
        ...
        for ( column = 0; column < [[childsPerRow objectAtIndex: row] unsignedIntValue]; column++, childIndex++)
        {
            ...
            TMVItemRenderer *childRenderer= [_childRenderers objectAtIndex: childIndex];
                ...
                rowsAreHoriz: (BOOL*) horizontal
{
    ...
    if ( [self weight] == 0 )
    {
        [rows addObject: [NSNumber numberWithUnsignedInt: 1]];
        [childsPerRow addObject: [NSNumber numberWithUnsignedInt: childCount]];
    ...
    else
    {
        *horizontal = _rect.size.width >= _rect.size.height;
        ...
        for ( i = 0; i < childCount; )
        {
            ...
            [rows addObject: [NSNumber numberWithDouble: rowHeight]];
            [childsPerRow addObject: [NSNumber numberWithUnsignedInt: childsUsed]];
            ...
            childWidths: (NSMutableArray*) childWidths
{
    ...
    *childsUsed = 0;
    ...
    {
        unsigned long long  childSize = [[_childRenderers objectAtIndex: i] weight];
        if ( childSize == 0)
        {
    ...
    *childsUsed = i - startChildIndex;
    ...
    // Now as we know thes rowHeight, we compute the widths of our children.
    for ( i = 0; i < *childsUsed; i++ )
    {
        // Rectangle(1.0 * 1.0) = mySize
        double rowSize = mySize * rowHeight;

        unsigned long long childSize= [[_childRenderers objectAtIndex: startChildIndex + i] weight];
        double cw= childSize / rowSize;
        //NSAssert(cw >= 0);

        [childWidths addObject: [NSNumber numberWithDouble: cw]];
    }
...
- (void) createChildRenderers
{
    ...
    for ( i = 0; i < childCount; i++ )
    {
        ...
        TMVItemRenderer *childRenderer = [[TMVItemRenderer alloc]
            initWithDataSource: _dataSource delegate: _delegate renderedItem: childItem tableMapView: _view];
